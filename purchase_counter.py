...
class PurchaseCounter:
    ...
    def __init__(self, path_to_file):
        ...
        self._total_count_by_each_purchases = {}

    def _add_purchase_to_total_count_by_each_purchases(self, purchase: str, purchase_count: int):
        if purchase not in self._total_count_by_each_purchases:
            self._total_count_by_each_purchases[purchase] = purchase_count
        else:
            self._total_count_by_each_purchases[purchase] += purchase_count
    ...
    def _calculate_total_count_by_each_purchases(self):
        with open(self._path_to_file, "r") as file_as_purchase_data:
            for row in file_as_purchase_data:
                try:
                    ...
                    self._add_purchase_to_total_count_by_each_purchases(purchase, purchase_count_as_int)
                except (ParseRowToPurchaseDataError, PurchaseCountIsNotIntegerError):
                    continue

    def _print_total_count_by_each_purchases(self):
